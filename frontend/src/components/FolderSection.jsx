...
import { ChevronDown, ChevronRight, Folder, Trash2, Plus, FolderPlus } from 'lucide-react';
import DeckCard from './DeckCard';
import Tooltip from './Tooltip';

const FolderSection = ({ folder, onDelete, onAddDeck, onAddSubfolder, selectMode = false, selectedDecks = new Set(), onToggleSelect }) => {
    ...
    <div className="mb-4">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-gray-400">({(folder.decks?.length || 0)} decks)</span>
        </div>
        {!selectMode && (
          <div className="flex items-center gap-1">
            <Tooltip text="Add subfolder" position="top">
              <button
                onClick={(e) => {
                  e.stopPropagation();
